### 应用界面
进入到打开的应用，用户可以看到左侧有一排选项，这里就介绍一下。深算工场的应用分为虚拟机和容器两种类型，应用界面左侧的菜单基本相同，但也有一些差异，下面介绍一下这两种类型应用的菜单。
...
![alt text](./appcentor25.png)

**（9）修改配额**：用户在使用应用过程中，若觉得配置不够影响运行速度，可以通过如下方式修改配额提升应用运行性能。在调整资源页面中，用户可以调整主机资源，包括：CPU、内存、磁盘、带宽，调整完毕需要重启应用才能配置生效。

![alt text](./appcentor26.png)

**（10）电源**：通过电源操作，可以对应用进行关机、重启和销毁。

![alt text](./appcentor27.png)
...
**（11）修改配额**：容器应用只能修改CPU、内存、磁盘、带宽的配额，虚拟机应用还可以新增一块或多块附加磁盘。

![alt text](./appcentor28.png)
