...
#### 概述
深算工场是一个领先的AI算力基座和云计算平台，在此平台上用户可以进行人工智能、大数据、云计算、区块链、后端开发等所有和计算机相关的训练和实验，用户可以方便地在平台上创建各种虚拟机环境进行操作，和在本地计算机上的体验是完全一致的，为用户节省了大量计算机硬件配置方面的消耗，大大降低了算力的开销。下面我们来解释一下算力和计费的相关概念和规则。
...
#### 2、算力模板
用户在深算工场上部署应用时需要选择算力服务（初级、中级、高级），其服务配置包括CPU、内存和云盘的算力计算，这里根据不同的应用模板使用了不同的算力模板，算力模板中配置的硬件和算力计算也是不同的。下表是深算工场目前使用的各种算力模板，包括：模板名称、模板配置、硬件型号、硬件类型、硬件单位、算力值、算力单位和总算力等。
<style>
  td {
